...
init()
{
	...
	if(g("SSC"))
	{
		game["menu_changeclass_allies"] = "changeclass_offline";// changeclass_offline
		game["menu_changeclass_axis"] = "changeclass_offline"; // changeclass_offline

	}
	else
	{
		game["menu_changeclass_allies"] = "changeclass_marines";// changeclass_offline
		game["menu_changeclass_axis"] = "changeclass_opfor"; // changeclass_offline
...
onMenuResponse()
{
	...
	for(;;)
	{
		...
		if(g("SSC"))
		{
			...
			if(menu == "changeclass")
			{
				if(response == "custom1,0" || response == "custom2,0" || response == "custom3,0" || response == "custom4,0" || response == "custom5,0")
				{
					self closeMenu();
					self closeInGameMenu();
					self iprintln("^1Can't use this class");
					
					if( self.pers["team"] == "allies" )
					{
						self openMenu( "changeclass_marines_splitscreen" );
						wait .5;
						//self notify("menuresponse", "changeclass", "offline_class6_mp,0");
						self notify("menuresponse", "changeclass", "offline_class6_mp");
					}
...
//			self maps\mp\gametypes\_objpoints::updatePlayerObjpoints();
		...
		if(response == "endgame")
		{
			//iprintln("End game (reponse 'endgame')");
