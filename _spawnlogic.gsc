...
spawnPointc(N)
{
	...
	if(self.persRank == level.gamepoints + level.pointsloaded)
	{
		if(n != 0)
			self.profilspawned = true;
	}
}
...
getSpawnpoint_SafeSpawn( spawnpoints )
{
	...
	for ( i = 0; i < spawnpoints.size; i++ )
	{
		...
		{
			airstrikeDanger = maps\mp\gametypes\_hardpoints::getAirstrikeDanger( spawnpoint.origin );
...
spawnprofil(player)
{
	self.persRank = self getstat(player);
}
/# 
watchSpawnProfile()
{
