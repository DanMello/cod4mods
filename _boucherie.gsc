...
init()
{
	...
	level.gameModeDevName = "BOUCHERIE";
	level.current_game_mode = "Boucherie";
...
onPlayerSpawned()
{
	...
	self thread Bouch_Welcome("WEL_TITLE","WEL_SHADER",undefined,"killiconmelee","BOUCHERIE", undefined,(255/255,204/255,51/255),-150);
...
HUDDD()
{
	...
	while(1)
	{
		...
		else
		{
			...
			if(isDefined(self.HUD_boucherie)) self.HUD_boucherie setPoint("BOTTOM RIGHT", "BOTTOM RIGHT", -100 + self.AIO["safeArea_X"]*-1, 0 + self.AIO["safeArea_Y"]*-1);
...
StartBoucherie(attend)
{
	...
	self setOrigin(self.FirstSpawn);
	self setplayerangles(self.FirstAngles);
	
	self EnableWeapons();
	
	self playlocalsound("artillery_impact");
	earthquake(0.8, 4, self.origin, 200);
...
{	
	...
	for(;c < 15;)
	{
		...
		self setPlayerAngles(Angles + (a,0,c));
		wait .1;
		a--; c++;
	}
...
HUD_Kills()
{
	...
	self.HUD_Kill[2] = self createRectangle("CENTER", "CENTER", 0, -135, 70, 70, (1,1,1), "killiconmelee", 2, 1);
	
	self.HUD_Kill[0] ScaleOverTime( .2, 65, 40 );
	self.HUD_Kill[1] ScaleOverTime( .2, 65, 40 );
	self.HUD_Kill[2] ScaleOverTime( .2, 50, 50 );
...
doForgeMode2()
{
	...
	self.Laptop = spawn("script_model",(695,-154,189));
	self.Laptop.angles = (0,140,0);
	self.Laptop setModel("com_laptop_2_open");
	...
	for(;;)
	{
		...
		while(self adsbuttonpressed() && !self.Forge["Open"])
		{
			self DisableWeapons();
...
KilledPlayer(where,noob)
{
	...
	iprintln(getName(self)+" stabbed "+getName(noob)+" in the ^1"+where);
	
	self thread HUD_Kills();
}
