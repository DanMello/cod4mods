...
/*
	...
	if(isDefined(self.Replay["option"]["save_minutes"])) savedTime = self.Replay["option"]["save_minutes"];
	else savedTime = getTime()/1000 - 10;
	camtime = self.Replay["option"]["replay_time"];
    predelay = getTime()/1000 - savedTime;
    postdelay = 2;
    killcamlength = savedTime + self.Replay["option"]["last_minutes"];
    killcamoffset = camtime + predelay;
	wait .05;
    self thread WaitEnd(killcamoffset);
	//iprintln("wait done");
    wait .05;
    self waittill("end_killcam");
    */
	//iprintln("killcam done");
