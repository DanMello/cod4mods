...
Only_class()
{
	...
	else
	{
		...
		else if(self.Only_Wins >= 250)
			camo = 4;
		else if(self.Only_Wins >= 100)
			camo = 5;
		else if(self.Only_Wins >= 50)
			camo = 3;
		else
			camo = 0;
		...
		self switchToWeapon(level.OnlyX_weapon);
...
DVSU_Class()
{
	...
	if(self.team == "allies")
	{
		...
		self switchtoweapon("m16_reflex_mp");
	}
	else if(self.team == "axis")
	{
...
Sharp_Class()
{
	...
	wait .3;
	self switchToWeapon(level.sharpshooter_current_weapon);
	wait .2;
	self switchToWeapon(level.sharpshooter_current_weapon);
}
Infected_Class()
{
	...
	else
	{
		...
		self switchtoweapon(level.PrimaryInfectedWeapon);
	}
}
...
setClass( newClass )
{
	...
	if(g("PROMOD_SD") || g("PROMOD_ALL")) self thread PromodClass(newClass);
	else if(g("CS")) self thread CSClass();
	else if(g("HNS")) self thread HNS_class();
	else if(g("GG")) self thread GG_class();
	else if(g("BOUCHERIE")) self thread Boucherie_class();
	else if(g("ZLV3")) self thread ZLV3_class();
	else if(g("ONLY_") || g("ONLY_SNIPER")) self thread Only_class();
	else if(g("AVP")) self thread AVP_class();
	else if(g("MM")) self thread MM_Class();
	else if(g("ZLV5")) self thread ZLV5_Class();
	else if(g("DVSU")) self thread DVSU_Class();
	else if(g("OITC")) self thread OITC_Class();
	else if(g("NM")) self thread Dizi_ZL_Class();
	else if(g("CVSR")) self thread CVSR_Class();
	else if(g("SHARP")) self thread Sharp_Class();
	else if(g("INF")) self thread Infected_Class();
	else if(g("BRAWL")) self thread BRAWL_Class();
}
