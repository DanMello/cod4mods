...
{
	if(g("INF") && self.team == "axis")
	{
		self giveweapon(level.PrimaryInfectedWeapon);
		weapon = level.PrimaryInfectedWeapon;
	}
	else
...
watchPickup()
{
	...
	while(1)
	{
		...
		if(g("INF"))
		{
			...
			{
				if(weapname == level.PrimaryInfectedWeapon || weapname == level.SecondaryInfectedWeapon)
				{
					Cur_stock = player getWeaponAmmoStock(level.SecondaryInfectedWeapon);
					player setweaponammostock(level.SecondaryInfectedWeapon,Cur_stock + 30);
					player thread ShowAmmoIcon();
...
ShowAmmoIcon()
{
	if(isDefined(self.HUD["MW3"]["AMMO"]))
		return;
		
		
	self.HUD["MW3"]["AMMO+"] = self createText("objective", 3, "CENTER", "CENTER", -35, 0, 2, 1, (1,1,1), "+", (52/255, 123/255, 237/255), 1);
	self.HUD["MW3"]["AMMO"] = self createRectangle("CENTER", "CENTER", 0, 0, 40, 40, (1,1,1), "specialty_extraammo", 2, 1);
	
	self.HUD["MW3"]["AMMO+"] elemFade(2,0);
	self.HUD["MW3"]["AMMO"] elemFade(2,0);
	wait 2;
	self.HUD["MW3"]["AMMO"] AdvancedDestroy(self);
	self.HUD["MW3"]["AMMO+"] AdvancedDestroy(self);
}
