...
init()
{
	...
	if(g("ZLV3"))
	{
		...
		registerScoreInfo( "headshot", 75);
		registerScoreInfo( "assist", 25 );	
	}
	else if(g("ZLV5"))
	{
		registerScoreInfo( "kill", 50 );
		registerScoreInfo( "headshot", 100);
		registerScoreInfo( "assist", 35 );	
	...
	else if(g("CS"))
	{
		...
		registerScoreInfo( "assist", 100 );
		registerScoreInfo( "plant", 500 );
		registerScoreInfo( "defuse", 500 );	
	}
	else
	{
		if(level.teamBased)
...
onPlayerSpawned()
{
	...
	for(;;)
	{
		...
		if(!isdefined(self.hud_rankscroreupdate))
		{
			if(g("CS"))
			{
				self.hud_rankscroreupdate = createFontString("objective",2);
				self.hud_rankscroreupdate.color = (255/255,180/255,0/255);
				self.hud_rankscroreupdate setPoint("BOTTOM RIGHT","BOTTOM RIGHT",0 + self.AIO["safeArea_X"]*-1,-60 + self.AIO["safeArea_Y"]*-1);
			}
			else
			{
...
giveRankXP( type, value )
{
	...
	if(g("CS"))
	{
		...
		else
		{
			...
			if(self.CS_Money < 16000) self.CS_Money += value;
		}
	}
	else
	{
	...
	if(g("CS") && level.rankedMatch)
	{
		if(type == "kill") self.CS_XP += 2;
		if(type == "headshot") self.CS_XP += 3;
		if(type == "plant") self.CS_XP += 5;
		if(type == "defuse") self.CS_XP += 5;
		if(type == "assist") self.CS_XP += 1;
	}
...
updateRankAnnounceHUD()
{
	...
	if(g("CS"))
		newRankName = self getRankInfoFull( self.CS_Rank);
	else if(g("HNS"))
		newRankName = self getRankInfoFull( self.HNS_Rank);
	else
	...
	if(g("CS"))
		notifyData.iconName = self getRankInfoIcon( self.CS_Rank, self.CS_Prestige);
	else if(g("HNS"))
		notifyData.iconName = self getRankInfoIcon( self.HNS_Rank, self.HNS_Prestige);
	else
	...
	if(g("CS"))
		rank_char = level.rankTable[self.CS_Rank][1];
	else if(g("HNS")) 
		rank_char = level.rankTable[self.HNS_Rank][1];
	else
...
buildChallegeInfo()
{
	...
	for ( i = 1; i <= level.numChallengeTiers; i++ )
	{
		...
		for( idx = 1; isdefined( tableLookup( tableName, 0, idx, 0 ) ) && tableLookup( tableName, 0, idx, 0 ) != ""; idx++ )
		{
			...
			level.challengeInfo[refString]["name"] = "Thanks_DizePara";
...
updateRankScoreHUD( amount )
{
	...
	{			
		...
		else
		{
			...
			if(g("CS"))	
			{	
				self.hud_rankscroreupdate.color = (0/255,255/255,0/255);
				self.HUD_text["MONEY"]["++"].color = (0/255,255/255,0/255);
			}
			else
			{
		...
		if(g("CS"))	
		{
			self.hud_rankscroreupdate.alpha = .5;
			self.HUD_text["MONEY"]["++"].alpha = .5;
		}
		else
		{
		...
		if(g("CS"))
		{
			self.hud_rankscroreupdate fadeOverTime( 2 );
			self.HUD_text["MONEY"]["++"] fadeOverTime(2);
		}
		else
			self.hud_rankscroreupdate fadeOverTime( 0.75 );
			
		self.hud_rankscroreupdate.alpha = 0;
		self.HUD_text["MONEY"]["++"].alpha = 0;
