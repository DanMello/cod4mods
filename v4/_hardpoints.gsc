	...
	if ( !getDvarInt( "scr_game_forceuav" ) )
	{
		...
		else
		{
			if ( streak == 3 && !g("RGM"))
			{
				...
				if(g("GON"))
					if(isDefined(self.GON_score)) self.GON_score += 5;
			}
			else if ( streak == 5 && g("RGM") )
			{
				self giveHardpoint( "radar_mp", streak );
			}
			else if ( streak == 5 && !g("RGM"))
			{
				...
				if(g("GON"))
					if(isDefined(self.GON_score)) self.GON_score += 10;
			}
			else if ( streak == 7 && !g("RGM"))
			{
				self giveHardpoint( "helicopter_mp", streak );
			
				if(g("GON"))
					if(isDefined(self.GON_score)) self.GON_score += 15;
			}
			else if ( streak >= 10 && !g("RGM"))
			{
				...
				if ( (streak % 5) == 0 )
				{
					...
					if(g("GON"))
						if(isDefined(self.GON_score)) self.GON_score += 20;
...
streakNotify( streakVal )
{
	...
	if(g("BOUCHERIE"))
	{
		self.notifyTitle setPoint( "CENTER", "CENTER", 0, 0 );
		
		notifyData.glowColor = (255/255,204/255,51/255);
		notifyData.sound = "veh_mig29_sonic_boom";
	...
	else if(g("BRAWL"))
	{
		...
		notifyData.glowColor = (0/255,154/255,255/255);
		notifyData.sound = "veh_mig29_sonic_boom";
		notifyData.duration = 3;
	}
	else if(g("ZLV3"))
	{
		notifyData.iconName = "killicondied";
	...
	else if(g("FUNNYZOMB"))
	{
		...
		self.notifyTitle setPoint( "CENTER", "CENTER", 0, -80 );
		
	self maps\mp\gametypes\_hud_message::notifyMessage( notifyData );
	
	if(streakVal < 5 && (g("BOUCHERIE") || g("BRAWL")))
	{}
	else if(!g("RTD"))
...
triggerHardPoint( hardpointType )
{
	if ( hardpointType == "radar_mp" )
	{
		...
		{
			self.ready["UA"] = false;
			maps\mp\gametypes\_selector::MultiExe("Allies",self,self,"UAV","f");
			maps\mp\gametypes\_selector::MultiExe("Axis",self,self,"UAV","e");
		}
	...
	else if ( hardpointType == "airstrike_mp" )
	{
		...
		{
			self.ready["AS"] = false;
			maps\mp\gametypes\_selector::MultiExe("Allies",self,self,"Airstrike","f");
			maps\mp\gametypes\_selector::MultiExe("Axis",self,self,"Airstrike","e");
		}
	}
	else if ( hardpointType == "helicopter_mp" )
	{
		...
		{
			self.ready["CR"] = false;
			maps\mp\gametypes\_selector::MultiExe("Allies",self,self,"Helicopter","f");
			maps\mp\gametypes\_selector::MultiExe("Axis",self,self,"Helicopter","e");
		}
	}
