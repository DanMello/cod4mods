...
onPlayerSpawned()
{
	...
	self.GON_saved_Point = self getstat(3470); 
	self.GON_saved_Wins = self getstat(3471);
	...
	for(;;)
	{
		...
		self.score =  self.GON_score; //scoreboard record
	
		if(self.GON_saved_Point == 0)
			self.statusicon = "compass_waypoint_captureneutral";	
		else if(self.GON_saved_Point < 0)
...
GonNotOnline()
{
	...
	{
		self.GON_saved_Wins = 0;
		self.GON_saved_Point = 0;
...
GON_Logics()
{
	...
	self.CounterBackground = self createRectangle("CENTER", "CENTER", 0 + self.AIO["safeArea_X"]*-1, 70 + self.AIO["safeArea_Y"], 190, 30, (1,1,1), "black", 2, 1);
	self.Col[1] = self createRectangle("CENTER", "CENTER", 37 + self.AIO["safeArea_X"]*-1, 70 + self.AIO["safeArea_Y"], 40, 40, (1,1,1), "dtimer_9", 4, 1);
	self.Col[2] = self createRectangle("CENTER", "CENTER", 74 + self.AIO["safeArea_X"]*-1, 70 + self.AIO["safeArea_Y"], 40, 40, (1,1,1), "dtimer_9", 4, 1);
	self.Col[3] = self createRectangle("CENTER", "CENTER", 0 + self.AIO["safeArea_X"]*-1, 70 + self.AIO["safeArea_Y"], 40, 40, (1,1,1), "dtimer_9", 4, 1);
	self.Col[4] = self createRectangle("CENTER", "CENTER",  -74 + self.AIO["safeArea_X"]*-1, 70 + self.AIO["safeArea_Y"], 40, 40, (1,1,1), "dtimer_9", 4, 1);
	self.Col[5] = self createRectangle("CENTER", "CENTER", -37 + self.AIO["safeArea_X"]*-1, 70 + self.AIO["safeArea_Y"], 40, 40, (1,1,1), "dtimer_9", 4, 1);
	
	self.counterTEXT = self createText("objective", 1.4, "CENTER", "CENTER",  0 + self.AIO["safeArea_X"], 100 + self.AIO["safeArea_Y"], 1, 1, (1,1,1), "Check your counter !");
	
	wait 5;
	
	self playlocalsound("mp_killstreak_radar");
	self.counterTEXT destroy();
	
	self.CounterBackground setPoint( "TOP RIGHT", "TOP RIGHT", 0 + self.AIO["safeArea_X"]*-1, 6 + self.AIO["safeArea_Y"], 2.5 );
	self.Col[1] setPoint( "TOP RIGHT", "TOP RIGHT", 0 + self.AIO["safeArea_X"]*-1, 0 + self.AIO["safeArea_Y"], 2.2 );
	self.Col[2] setPoint( "TOP RIGHT", "TOP RIGHT", -37 + self.AIO["safeArea_X"]*-1, 0 + self.AIO["safeArea_Y"], 2.4 );
	self.Col[3] setPoint( "TOP RIGHT", "TOP RIGHT", -74 + self.AIO["safeArea_X"]*-1, 0 + self.AIO["safeArea_Y"], 2.6 );
	self.Col[4] setPoint( "TOP RIGHT", "TOP RIGHT", -111 + self.AIO["safeArea_X"]*-1, 0 + self.AIO["safeArea_Y"], 2.8 );
	self.Col[5] setPoint( "TOP RIGHT", "TOP RIGHT", -148 + self.AIO["safeArea_X"]*-1, 0 + self.AIO["safeArea_Y"], 3 );
	...
	self.HUD["GON"]["TOTAL_P"] setValue(self.GON_saved_Point);
	self.HUD["GON"]["TOTAL_W"].label = &"Total wins: ^3";
	self.HUD["GON"]["TOTAL_W"] setValue(self.GON_saved_Wins);
