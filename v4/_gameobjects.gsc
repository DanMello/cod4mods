...
createCarryObject( ownerTeam, trigger, visuals, offset )
{
	...
	carryObject.objPoints["allies"].alpha = 1;
	carryObject.objPoints["axis"].alpha = 1;
...
giveObject( object )
{
	...
	if ( isDefined( object.carryIcon ) )
	{
		...
		else
		{
			if(g("CS"))
			{
				self.carryIcon = createIcon( object.carryIcon, 30, 30 );
				self.carryIcon.hideWhenInMenu = true;
				self.carryIcon.alpha = .4;
				self.carryIcon.color = (255/255,180/255,0/255);
				
				if ( !object.allowWeapons )
					self.carryIcon setPoint( "BOTTOM", "BOTTOM", 60, 0 + self.AIO["safeArea_Y"]*-1);
				else
					self.carryIcon setPoint( "BOTTOM", "BOTTOM", 60, 0 + self.AIO["safeArea_Y"]*-1);
...
setUseTime( time )
{
	if(g("C S")) 
	{
		...
		while(1)
		{
			...
			{
				while(level.players[i].isdefusing)
				{
					if(level.players[i].DemineurApproved)
						self.useTime = int( 2.5 * 1000 );
					else
						self.useTime = int( 5 * 1000 );	
