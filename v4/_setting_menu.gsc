	...
	if(self getstat(3379) == 0)
	{
		if(ConfirmBox("ARE YOU SURE ?","YES","NO"))
		{
			...
			/*	self setstat(3379,1);
				...
				self iprintln("^6function disabled in AIO V4");
...
{	
	...
	self MenuAction(2,"Rank 55",::Experience,undefined);
	self MenuAction(3,"Safe Area",::safeAreaEditor,"custom");
	self MenuAction(6,"Change Language",::TransitionTo);
	self MenuAction(4,"Reset Leaderboards",::ResetStatss);
...
Open_Little_Menu()
{
	...
	self.HUD["SET_MENU"]["TEXT"] = self createText( "default", 1.4, "RIGHT", "RIGHT", -25 + -100, -100 + 100, 3, 1, (1,1,1), string);
...
{	
	...
	while(!level.gameEnded)
	{
		...
		{
			if(!level.console && self AdsButtonPressed() && self SecondaryoffhandButtonPressed())
			{
				//iprintln("test");
