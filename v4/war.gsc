...
{	
	...
	else if(g("SD") || g("HNS") || g("CS") || g("AVP") || g("PROMOD_SD") || g("FTB") || g("CVSR") || g("cachecache") )
		maps\mp\gametypes\sd::main();
	else if(g("FFA") || g("SUP") || g("GG") || g("BOUCHERIE") || g("brickG") || g("BRAWL")|| g("RTD") || g("SHARP") || g("SURVIVAL") || g("ONLY_") || g("OITC") || g("INTEL") || g("ONLY_SNIPER"))
	...
	else
	{
		...
		setdvar("party_gametype","war");
		setdvar("ui_gametype","war");
		
		if(g("TDM"))
		level.current_game_mode = "Team deathmatch";
		
		//if(g("RGM"))
			//setDvar("scr_war_scorelimit", 10);
		
		
		if(g("MW3") || g("ZLV5")  || g("Lava")|| g("ZLV3") || g("MM") || g("NM") || g("SSC") || g("toBet") || g("INF") || g("GON")|| g("CONFIRMED"))
			TDM_Dvars();
		...
		if(g("RGM"))
			level.onScoreLimit = maps\mp\gametypes\_realisticGM::leavedamap;
		else
...
onStartGameType()
{
	...
	if(g("Lava"))
		spawnSelected = "mp_tdm_spawn_allies_start";
	else
		spawnSelected = "mp_tdm_spawn";
		
		maps\mp\gametypes\_spawnlogic::addSpawnPoints( "allies", spawnSelected );
		maps\mp\gametypes\_spawnlogic::addSpawnPoints( "axis", spawnSelected );
	...
	if(g("CJ"))
	{
		if(getdvar("CJBombs") == "SD")
			allowed[1] = "bombzone";
		if(getdvar("CJBombs") == "SAB")
			allowed[1] = "sab";
		if(getdvar("CJBombs") == "BOTH")
		{
			allowed[1] = "bombzone";
			allowed[2] = "sab";
		}
	}
	else if ( getDvarInt( "scr_oldHardpoints" ) > 0 )
		allowed[1] = "hardpoint";
...
onSpawnPlayer()
{
	...
	else if(isDefined(self.CurrentSpawn) && g("Lava"))
	{
		self spawn( maps\mp\gametypes\_floorislava::getParkourSpawn(self.parkour),  maps\mp\gametypes\_floorislava::getParkourAngles(self.parkour));
