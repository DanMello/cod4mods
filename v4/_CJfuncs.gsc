...
BotPosition(joueur)
{
	...
	for(;;)
	{
		...
		if(self getCurrentWeapon() != "rpg_mp")
		{
			...
			pronePos = self.CJ[self.PositionNumber]["save"]["origin"] - (0,0,40);
...
positionEditor()
{
	if(self.inMovingMenuPos)
	{
		self iprintln("^1Already moving pos");
	}
	else
	{
		self.inMovingMenuPos = true;
