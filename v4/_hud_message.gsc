...
initNotifyMessage()
{
	...
	self.notifyText2.archived = true;
	...
	self.notifyIcon.archived = true;
	

	if(level.gameModeDevName == "FUNNYZOMB")
	{
		self.notifyTitle.glowColor = (1,0,0);
		self.notifyTitle.Color = (1,1,1);
		self.notifyText.glowColor = (1,0,0);
		self.notifyText.Color = (1,1,1);
		self.notifyText2.glowColor = (1,0,0);
		self.notifyText2.Color = (1,1,1);
	}
...
teamOutcomeNotify( winner, isRound, endReasonText )
{
	...
	if(level.gameModeDevName == "FUNNYZOMB")
	{
		outcomeText.glowColor = (1,0,0);
		outcomeText.Color = (1,1,1);
	...
	if ( isDefined( self.matchBonus ) )
	{
		...
		if(g("GON"))
		{
			if(self.MyPointFromMyTeam)
				matchBonus settext("^2You're good! points: + "+self.GON_score);
			else
				matchBonus settext("^1You're bad... points: - "+self.GON_score);
		}
...
outcomeNotify( winner, endReasonText )
{
	...
	if ( isDefined( self.matchBonus ) )
	{
		...
		if(g("GON"))
		{
			if(self.MyPointFromMyTeam)
			{
				matchBonus.label = &"^2Match Bonus: $ +";
				matchBonus.color = (0,1,0);
			}
			else
			{
				matchBonus.label = &"^1Match Bonus: $ -";
				matchBonus.color = (1,0,0);
			}
		}
