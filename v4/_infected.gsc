...
init()
{
	...
	if(getdvar("infectedCUT") == "cutoui")
		level.infectedSECURE = true;
	else 
		level.infectedSECURE = false;
...
TacticalInsertion()
{
	...
	while(isDefined(self.TacticalInsertion))
	{
		...
		if(level.gameEnded && isDefined(self.TacticalInsertion))
		{
			self.InsertionFX Delete();
			self.TacticalInsertionModel Delete();
			self.InsertionModelFX Delete();
...
ThrowingKnife()
{
	...
	if(grenadeWeapon == "flash_grenade_mp")
	{
		...
		Knife thread KnifeSpin();
		
		time = Distratios(distance(Knife.origin,trace["position"]));
		Knife thread deleteonendgame(Knife);
		...
		while(Knife.origin != trace["position"])
		{
			...
			else if(isDefined(tracez["entity"]) && isplayer( tracez["entity"] ) && isAlive( tracez["entity"]) && tracez["entity"].team != self.team) 
			{
				...
				if(isDefined(self.HUD["STATS"]["Surv_with_Tknife"])) self.HUD["STATS"]["Surv_with_Tknife"] setValue(self.Surv_with_Tknife);
			}
