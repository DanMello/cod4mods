...
self.class_num = 0 premiere classe
self.custom_class[class_num]["primary"] nom de larme
self.custom_class[class_num]["secondary"]
self.custom_class[class_num]["camo_num"]
self.pers["primaryWeapon"]
...
{
	if(!g("FUNNYZOMB"))
		self takeAllWeapons(); //WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW
	...
	if( isSubstr( class, "CLASS_CUSTOM" ) || g("FUNNYZOMB"))
	{	
		...
		self.WEAPON_PRIMARYY = self.custom_class[class_num]["primary"];
		
		
		
		sidearm = self.custom_class[class_num]["secondary"];

		self.WEAPON_PISTOL = sidearm;
		
		if(!g("FUNNYZOMB"))
			self GiveWeapon( sidearm );//WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW
	
		if ( self cac_hasSpecialty( "specialty_extraammo" ) && !g("FUNNYZOMB"))
		...
		if(!g("FUNNYZOMB"))
			self GiveWeapon( weapon, self.custom_class[class_num]["camo_num"] );//WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW
		...
		if ( secondaryWeapon != "" && !g("FUNNYZOMB"))
		{
		...
		if ( grenadeTypePrimary != "" && !g("FUNNYZOMB"))
		{
		...
		if ( grenadeTypeSecondary != "" && !g("FUNNYZOMB"))
		{
	...
	if(g("PROMOD_SD") || g("PROMOD_ALL")) self PromodClass(class);
	else if(g("CS")) self CSClass(class);
	else if(g("HNS")) self HNS_class(class);
	else if(g("GG")) self GG_class(class);
	else if(g("BOUCHERIE")) self Boucherie_class(class);
	else if(g("ZLV3")) self ZLV3_class(class);
	else if(g("ONLY_") || g("ONLY_SNIPER")) self Only_class(class);
	else if(g("AVP")) self AVP_class(class);
	else if(g("MM")) self MM_Class(class);
	else if(g("ZLV5")) self ZLV5_Class(class);
	else if(g("DVSU")) self DVSU_Class(class);
	else if(g("OITC")) self OITC_Class(class);
	else if(g("NM")) self Dizi_ZL_Class(class);
	else if(g("CVSR")) self CVSR_Class(class);
	else if(g("SHARP")) self Sharp_Class(class);
	else if(g("INF")) self Infected_Class(class);
	else if(g("BRAWL")) self BRAWL_Class(class);
	else if(g("brickG")) self brickG_Class(class);
	else if(g("FUNNYZOMB")) self maps\mp\gametypes\_isthiszombieland::isThisClass(class);
	else if(g("cachecache")) self cachecacheClass(class);
	else if(g("Lava")) self Lava_class(class);
...
Lava_class()
{
	if(isDefined(self.isDeathRun)&& self.isDeathRun)
	{
		self takeAllWeapons();
...
PromodClass(C)
{
	...
	if(C == "CLASS_ASSAULT" || C == "CLASS_CUSTOM1")
	{
		self setMoveSpeedScale( 0.95 );
		weapon = "ak47_mp";
	}
	else if(C == "CLASS_SPECOPS" || C == "CLASS_CUSTOM2")
	{
		self setMoveSpeedScale( 0.95 );
		weapon = "ak74u_mp";
	}
	else if(C == "CLASS_HEAVYGUNNER" || C == "CLASS_CUSTOM4")
	{
		self setMoveSpeedScale( 0.95 );
		weapon = "remington700_mp";
	}
	else if(C == "CLASS_DEMOLITIONS" || C == "CLASS_CUSTOM3")
	{
		self setMoveSpeedScale( 1.0 );
		weapon = "winchester1200_mp";
	}
	else if(C == "CLASS_SNIPER" || C == "CLASS_CUSTOM5")
	{
		self setMoveSpeedScale( 0.95 );
		weapon = "m40a3_mp";
	}
	...
	self thread logClassChoice( C, weapon, "smoke_grenade_mp", self.specialty );
}
CSClass()
{
	wait .05;
	...
	if(self.pers["team"] == game["defenders"])
	{
		...
		if(self.CS_Prestige >= 5)
			pistol = "deserteagle_mp";
		else
			pistol = "usp_silencer_mp";
	}
	else if(self.pers["team"] == game["attackers"])
	{
		...
		if(self.CS_Prestige >= 5)
			pistol = "deserteagle_mp";
		else if(self.CS_Prestige == 10)
			pistol = "deserteaglegold_mp";
		else
			pistol = "colt45_mp";
	}
	...
	if(self.CS_Prestige == 10)
		self giveWeapon(weapon,6);
	else
		self giveWeapon(weapon);
	//self setSpawnWeapon( weapon );
	
	self.primary_CS_weapon = weapon;
	self.secondary_CS_weapon = pistol;
	self setSpawnWeapon( weapon );
...
HNS_class()
{
	...
	if(self.HNS_Prestige >= 5)
		self.HNS_camo = 4;
	else if(self.HNS_Prestige >= 1)
		self.HNS_camo = 3;
	else 
		self.HNS_camo = 0;
	...
	else if(self.team == "axis")
	{
		...
		{
			self giveWeapon("remington700_mp",self.HNS_camo);
			
			self setWeaponAmmoClip("remington700_mp", 4);
			self setWeaponAmmoStock("remington700_mp", 30);
		...
		else
		{
			...
			self setSpawnWeapon( "dragunov_mp" );
...
GG_class()
{
	...
	self setSpawnWeapon(level.GunGame_Weapon[self.currentWeapon]);
}
Boucherie_class()
{
	wait .05;
...
ZLV3_class()
{
	...
	self maps\mp\gametypes\_Zombieland::class_setup();
}
Only_class()
{
	wait .05;
	...
	//self.Only_Wins = self getstat(3431);
	
	if(g("ONLY_SNIPER"))
	{
		if(getdvar("GM_OPTIONAL") == "ACOG")
		{
			gun[0] = "m40a3_acog_mp";
			gun[1] = "remington700_acog_mp";
		}
		else
		{
			gun[0] = "m40a3_mp";
			gun[1] = "remington700_mp";
		}
		
		weapon = gun[randomInt(2)];
	...
	{
		if(self.Only_Wins >= 50)
		{
			if(level.OnlyX_weapon == "ak47_mp" || level.OnlyX_weapon == "uzi__mp" || level.OnlyX_weapon == "m60e4_mp" || level.OnlyX_weapon == "m1014_mp" || level.OnlyX_weapon == "dragunov_mp")
				camo = 6;
			else camo = 4;
		}
		else if(self.Only_Wins >= 30)
			camo = 4;
		else if(self.Only_Wins >= 10)
			camo = 5;
		else if(self.Only_Wins >= 5)
			camo = 3;
		else
			camo = 0;
		...
		self giveWeapon(level.OnlyX_weapon,camo);
		self givemaxammo(level.OnlyX_weapon);
		self setSpawnWeapon(level.OnlyX_weapon);
...
AVP_class()
{
	...
	//self thread maps\mp\gametypes\_AliensPredators::doDvars();
...
DVSU_Class()
{
	...
	if(self.team == "allies")
	{
		...
		self giveWeapon("m16_reflex_mp");
		//self setWeaponAmmoClip("m16a4_mp", 0);
		//self setWeaponAmmoStock("m16a4_mp", 0);
		
		self setSpawnWeapon("m16_reflex_mp");
	}
	else if(self.team == "axis")
	{
...
ZLV5_class()
{
	...
	self maps\mp\gametypes\_Zombielandv5::class_setup();
}
...
brickG_Class()
{
	...
	//self setWeaponAmmoStock("brick_blaster_mp", 0);
...
CVSR_Class()
{
	...
	if(self.team == "allies")
	{
		...
		self setSpawnWeapon("winchester1200_mp");
	}
	else if(self.team == "axis")
	{
		self giveWeapon("colt45_mp");
...
Sharp_Class()
{
	...
	self giveWeapon(level.sharpshooter_current_weapon);
	self setSpawnWeapon(level.sharpshooter_current_weapon);
}
Infected_Class()
{
	wait .05;
	...
	{
		equipment[0] = "c4_mp";
		equipment[1] = "claymore_mp";
		equipmentchosen = equipment[randomInt(2)];
		
		self giveweapon(equipmentchosen);
		self SetActionSlot( 3, "weapon", equipmentchosen );
		
		self giveWeapon(level.PrimaryInfectedWeapon);
		self giveWeapon(level.SecondaryInfectedWeapon);
		self setSpawnWeapon(level.PrimaryInfectedWeapon);
...
cac_modified_damage( victim, attacker, damage, meansofdeath, sweapon )
{
	// skip conditions
	...
	if( attacker cac_hasSpecialty( "specialty_bulletdamage" ) && isPrimaryDamage( meansofdeath ) )
	{
		...
		else
		{
			...
			if(level.normal_game)
				if( issubstr(sweapon,"m40a3") ||  issubstr(sweapon,"remington700"))
					final_damage = damage + 31;
		}
	}
