...
Adv_settings(opt)
{
	if(opt == "jump")	
	{
		if(!self.JumpSlowDown)
		{
			...
			self iPrintln("jump_slowdownEnable [^2ON^7]");
			self.JumpSlowDown = true;
		}
		else
		{
			self setClientDvar("jump_slowdownEnable",0);
			self iPrintln("jump_slowdownEnable [^1OFF^7]");
	...
	else if(opt == "sprint")
	{
		if(self.UnlimitedSprint)
		{
			...
			self iPrintln("Sprint Unlimited [^1OFF^7]");
...
JusquauBoutdeLaNuit()
{
	self iPrintln("Game ^2Unlimited ^7!");
...
WhatIsPO()
{
	CallLowerText("Profil option is like the 'START CODJUMPER' on my old menu\nBut now you can choose what you want launch !\nFor that go to gameplay or graphic params and select what you want\n^3Note: ^7Your params will be saved for ever! You can turn donw your PS3 etc..",15);
}
...
GraphicFuncs(G)
{
	switch(G)
	{
		case "-1": self iprintlnbold("Total number of saved position: "+self.SavedPosition);
		wait 1;
		self iprintlnbold("Total number of loaded position: "+self.LoadedPosition);
		break;
		case "0": if(!self.ShowSLText)
		{
		...
		break;
		case "1": if(!self.fx_enable)
		{
			self setClientDvar("fx_enable",0);
			self iPrintln("fx_enable [^1OFF^7]");
			self.fx_enable = true;
		}
		else
		{
			self setClientDvar("fx_enable",1);
			self iPrintln("fx_enable [^2ON^7]");
			self.fx_enable = false;
		}
		break;
		case "2": if(!self.fx_marks)
		{
			self setClientDvar("fx_marks",0);
			self iPrintln("fx_marks [^1OFF^7]");
			self.fx_marks = true;
		}
		else
		{
			self setClientDvar("fx_marks",1);
			self iPrintln("fx_marks [^2ON^7]");
			self.fx_marks = false;
		}
		break;
		case "3": if(!self.fx_draw)
		{
			self setClientDvar("fx_draw",0);
			self iPrintln("fx_draw [^1OFF^7]");
			self.fx_draw = true;
		}
		else
		{
			self setClientDvar("fx_draw",1);
			self iPrintln("fx_draw [^2ON^7]");
			self.fx_draw = false;
		}
		break;
		case "4": if(!self.ShowIcons)
		{
		...
		else
		{
			...
			self iPrintln("Icons [^2ON^7]");
			self.ShowIcons = false;
		}
		break;
		case "5": if(!self.ShowGun)
		{
			self setClientDvar("cg_drawgun", 0);
			self iPrintln("cg_drawgun [^1OFF^7]");
			self.ShowGun = true;
		}
		else
		{
			self setClientDvar("cg_drawgun", 1);
			self iPrintln("cg_drawgun [^2ON^7]");
			self.ShowGun = false;
		}
		break;
		case "6": if(!self.ShowCrossHair)
		{
			self setClientDvar("cg_drawcrosshair", 0);
			self iPrintln("Cross hair [^1OFF^7]");
			self.ShowCrossHair = true;
		}
		else
		{
			self setClientDvar("cg_drawcrosshair", 1);
			self iPrintln("Cross hair [^2ON^7]");
			self.ShowCrossHair = false;
		}
		break;
		case "7": if(!self.Fov85)
		{
			self.Fov85 = true;
			self setClientDvar("cg_fov","85");
			self iPrintln("Fov 85 [^2ON^7]");
		}
		else
		{
			self.Fov85 = false;
			self setClientDvar("cg_fov","65");
			self iPrintln("Fov 85 [^1OFF^7]");
		}
		break;
		case "8": if(!self.ShowKillfeed)
		{
			self iPrintln("Killfeed [^1OFF^7]");
		...
		else
		{
			...
			self iPrintln("Killfeed [^2ON^7]");
			self.ShowKillfeed = false;
		}
		break;
		case "9": if(!self.ShowFPS)
		{
		...
		else
		{
			...
			self iPrintln("Ranks and Icons [^2ON^7]");
			self.ShowRankIcons = false;
		}
		break;
		case "11": if(!self.ShowNames)
		{
			self setClientDvar("cg_overheadNamesSize", 0);
			self iPrintln("Name [^1OFF^7]");
			self.ShowNames = true;
		}
		else
		{
			self setClientDvar("cg_overheadNamesSize", 0.5);
			self iPrintln("Name [^2ON^7]");
			self.ShowNames = false;
		}
		break;
		case "12": if(!self.ShowSpecButs)
		{
		...
		else
		{
			...
			self iPrintln("Spectator Buttons [^2ON^7]");
			self.ShowSpecButs = false;
		}
		break;
		case "-10": if(!self.ShowHardcore)
		{
...
Allweap(w)
{
	if(getDvar("scr_oldschool") == "0")
	{
		if(w == "ak47" || w == "g36c" || w == "g3" || w == "m14" || w == "m16_" || w == "m4" || w == "mp44")
		{
			self setMoveSpeedScale( 0.95 );
			self iprintln("MoveSpeedScale modified ! ^3Your weight is now a Assault rifle class");
		}
		else if(w == "mp5" || w == "uzi" || w == "p90" || w == "skorpion")
		{
		...
		else if(w == "winchester1200" || w == "m1014")
		{
			self setMoveSpeedScale( 1.0 );
			self iprintln("MoveSpeedScale modified ! ^3Your weight is now a Shotgun class");
		}
		else if(w == "m40a3" || w == "dragunov" || w == "barrett" || w == "m21" || w == "remington700" || w == "ak74u")
		{
			self setMoveSpeedScale( 0.95 );
	...
	if(w == "usp" || w == "beretta" || w == "colt45" || w == "deserteagle" || w == "deserteaglegold") self TakeWeapon(self.WEAPON_PISTOL);
	else self TakeWeapon(self GetCurrentWeapon());
...
GodMode(mode)
{
	...
	for(;;)
	{
		...
		{
			//if(self.maxhealth != 90000) self.maxhealth = 90000;
...
Savex1()
{
	...
	for(;;)
	{
		if(self meleebuttonpressed() && self isOnground() && !self.IN_MENU["CJ"])
		{
...
Loadx1()
{
	...
	for(;;)
	{
		if(self useButtonPressed() && !self.IN_MENU["CJ"])
		{
			if(self AdsButtonPressed() && self.Elebot)
			{
			}
			else if(!isDefined(self.CJ[self.PositionNumber]["save"]["origin"])) self iPrintln("^1Save Position First !");
			else
			{
...
Savex2()
{
	...
	for(;;)
	{
		if(self meleeButtonPressed())
		{
			...
			{
				if(catch_next && self meleeButtonPressed() && self isOnground() && !self.IN_MENU["CJ"])
				{
					self.CJ[self.PositionNumber]["save"]["origin"] = self.origin;
					self.CJ[self.PositionNumber]["save"]["angles"] = self getplayerangles();
					self.SavedPosition++;
...
Loadx2()
{
	...
	for(;;)
	{
		if(self useButtonPressed())
		{
			...
			for(i=0;i<=0.5;i+=0.05)
			{
				if(catch_next && self useButtonPressed() && !(self isMantling()) && !self.IN_MENU["CJ"])
				{
					if(!isDefined(self.CJ[self.PositionNumber]["save"]["origin"])) self iPrintln("^1Save Position First !");
					else
					{
						self setPlayerAngles(self.CJ[self.PositionNumber]["save"]["angles"]);
						self setOrigin(self.CJ[self.PositionNumber]["save"]["origin"]);
